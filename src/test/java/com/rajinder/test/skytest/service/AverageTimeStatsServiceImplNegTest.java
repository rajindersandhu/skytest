...
public class AverageTimeStatsServiceImplNegTest {
	...
	public static void beforeAll() {
		System.setProperty("inputFilePath", "src/test/resources/data/TestData2.csv");
		System.setProperty("outputFilePath", "testresults/TestDataResult.csv");
	}

	@Test
	public void testAvgStatsAsc() throws Exception {
		List<DriverTimeTuple> list = service.calculateStats(System.getProperty("inputFilePath"), true, 3, false);
	...
	public void testAvgStatsDesc() throws Exception {
		List<DriverTimeTuple> list = service.calculateStats(System.getProperty("inputFilePath"), false, 3, false);
		assertTrue(list.size() == 3);
		for (int i = 1; i < list.size(); i++) {
			assertTrue(list.get(i - 1).getTime().compareTo(list.get(i).getTime()) == 1);
	...
	public void testReadData() throws Exception {
		Map<String, List<BigDecimal>> map = service.readFileData(System.getProperty("inputFilePath"), false);
	...
	@Test
	public void testReadDataThrowsErrorOnInvalidRecord() throws Exception {
		assertThrows(InvalidRecordException.class,
				() -> service.readFileData(System.getProperty("inputFilePath"), true));
	}
}
